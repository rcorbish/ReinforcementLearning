...
class Camera :
    ...
    def __init__( self ) :
        self.viewMatrix = p.computeViewMatrix(
            cameraEyePosition=[0, 0, Camera.CameraHeight ],
    ...
    def move( self, pos, quat ) :
        rc = p.multiplyTransforms(
            pos,
            quat, 
            [1, 0, Camera.CameraHeight],
            [0,0,0,1]
        )
        tgt = rc[0]

        self.viewMatrix = p.computeViewMatrix(
            cameraEyePosition=[ pos[0], pos[1], Camera.CameraHeight ],
